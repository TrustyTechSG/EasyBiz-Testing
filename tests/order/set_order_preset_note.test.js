...
await page.getByRole('button', { name: 'preset notes' }).click();
...
await expect(page.getByRole('tabpanel', { name: 'Customer receipt' }).getByText('** preset notes')).toBeVisible();
await expect(page.getByRole('tabpanel', { name: 'Customer receipt' }).getByText('Subtotal 2/pcs ₹ 22.00')).toBeVisible();
await expect(page.getByRole('tabpanel', { name: 'Customer receipt' }).getByText('GST 2% ₹ 0.43')).toBeVisible();
await expect(page.getByRole('tabpanel', { name: 'Customer receipt' }).getByText('Rounding ₹ 0.01')).toBeVisible();
await expect(page.getByRole('tabpanel', { name: 'Customer receipt' }).getByText('Total (inclusive of GST) ₹ 22.00')).toBeVisible();
await expect(page.getByRole('tabpanel', { name: 'Customer receipt' }).getByText('Pay by Cash (₹ 22.00)')).toBeVisible();
