...
test('test', async ({ page }) => {
  ...
  await page.getByText('sqft').click();
  ...
  await page.getByRole('button', { name: 'edit 1 sqft' }).click();
  await page.getByRole('button', { name: '1', exact: true }).click();
  await page.getByRole('button', { name: '1', exact: true }).click();
  await page.getByRole('button', { name: 'Update' }).click();
  ...
  await expect(page.getByText('Mattress Protector ₹ 11.00 x 11/sqft ₹ 121.00')).toBeVisible();
  await expect(page.getByText('- Laundry')).toBeVisible();
  await expect(page.getByText('- 1 pcs')).toBeVisible();
  await expect(page.getByText('Total (inclusive of GST) ₹ 120.95')).toBeVisible();
});