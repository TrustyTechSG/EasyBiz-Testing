import { test, expect, waitFor } from '@playwright/test';
...
test('test', async ({ page }) => {
  ...
  await waitFor(() => expect(page.getByText(`- Estimated completion: ${formattedNextMonthDate} (Su)`)).toBeAttached());
  await waitFor(() => expect(page.getByText('- Coating ₹ 50.00')).toBeAttached());
  await waitFor(() => expect(page.getByText('** default colour')).toBeAttached());
  await waitFor(() => expect(page.getByText('- Brand: MULBERRY')).toBeAttached());
  await waitFor(() => expect(page.getByText('- Serial No.: 5')).toBeAttached());
  await waitFor(() => expect(page.getByText('- Colour: METALLIC SILVER')).toBeAttached());
  await waitFor(() => expect(page.getByText('Subtotal 1/pcs ₹ 50.00')).toBeAttached());
  await waitFor(() => expect(page.getByText('Total (inclusive of GST) ₹ 51.00')).toBeAttached());
  await waitFor(() => expect(page.getByText('Pay by Cash (₹ 51.00)')).toBeAttached());
  await waitFor(() => expect(page.getByText('Return type: Self collection at FIRST STORE')).toBeAttached());
